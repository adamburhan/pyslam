...
class FeatureTrackerConfigs:
    ...
    SIFT = dict(
        num_features=kNumFeatures,  # independently computes the number of octaves
    ...
    )  # <= Using XFEAT matcher here!

    XFEAT_LIGHTGLUE = dict(
        num_features=kNumFeatures,  # N.B.: here, keypoints are not oriented! (i.e. keypoint.angle=0 always)
        num_levels=1,
        scale_factor=1.2,
        detector_type=FeatureDetectorTypes.XFEAT,
