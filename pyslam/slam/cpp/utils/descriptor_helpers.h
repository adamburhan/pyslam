...
*/
inline int convert_cv2_colormap_type_to_int(const cv::ColormapTypes &cv2_colormap_type) {
    return static_cast<int>(cv2_colormap_type);
}

inline cv::ColormapTypes convert_int_to_cv2_colormap_type(int colormap_type_int) {
    if (colormap_type_int < cv::ColormapTypes::COLORMAP_AUTUMN ||
        colormap_type_int > cv::ColormapTypes::COLORMAP_TWILIGHT_SHIFTED) {
        MSG_ERROR("convert_int_to_cv2_colormap_type_safe: Invalid colormap type");
        throw std::out_of_range("convert_int_to_cv2_colormap_type_safe: Invalid colormap type");
    }
    return static_cast<cv::ColormapTypes>(colormap_type_int);
}
